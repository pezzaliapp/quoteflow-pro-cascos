...
import { VEHICLE_TYPES, FLOOR_TYPES, selectProducts } from './data/rules.js';
import {
  ChevronRight, RotateCcw, FileText, ShoppingCart, Building2,
  Truck, Download, CheckCircle2, ArrowLeft, AlertCircle, Upload,
  Package, Euro, Gauge, Wrench, Copy, MessageCircle, Printer
} from 'lucide-react';
        ...
        try {
          ...
          const updated = [...productsData].map(p => {
            for (let i = headerIdx + 1; i < rows.length; i++) {
              const row = rows[i];
              if (String(row[colRef] || '').trim() === String(p.codice)) {
                const v = parseFloat(String(row[colPre] || '').replace(/[^\d.]/g, ''));
                if (!isNaN(v)) return { ...p, prezzoNetto: v };
              }
            }
            return p;
          });
...
function generateWhatsAppText({ customer, product, qty, sconto, scontoEuro, prezzoTotale, prezzoFinale, note, config, mode, docType }) {
  ...
  const lines = [
    `🔧 *CASCOS LIFTS — ${docType}*`,
    `📅 Data: ${today()}`,
    ``,
    `👤 *Cliente*`,
    `Nome: ${customer.nome}`,
    customer.azienda ? `Azienda: ${customer.azienda}` : null,
    customer.telefono ? `Tel: ${customer.telefono}` : null,
    customer.email ? `Email: ${customer.email}` : null,
    customer.indirizzo ? `Indirizzo: ${customer.indirizzo}` : null,
    ``,
    `🚗 *Configurazione*`,
    `Pavimentazione: ${floorLabel}`,
    `Tipo veicolo: ${vehicleInfo?.label}`,
    ``,
    `🏗️ *Prodotto Selezionato*`,
    `Modello: *${product.modello}*`,
    `Riferimento: ${product.codice}`,
    ...
    `Quantità: ${qty}`,
    qty > 1 ? `Subtotale: ${formatPrice(prezzoTotale)}` : null,
    sconto > 0 ? `Sconto ${sconto}%: -${formatPrice(scontoEuro)}` : null,
    `*TOTALE NETTO: ${formatPrice(prezzoFinale)}*`,
    `_(IVA esclusa)_`,
    note ? `\n📝 *Note*\n${note}` : null,
    ``,
    `─────────────────────`,
    `Cormach Srl — Distribuzione Cascos`,
    `www.alessandropezzali.it/quoteflow-pro-cascos`,
  ];
  ...
  return (
    <div
      ...
      className={`relative rounded-xl p-5 cursor-pointer transition-all duration-200 animate-slide-up
        ${isRecommended ? 'glass border-blue-500/50 ring-1 ring-blue-500/30 hover:ring-blue-400/60' : 'glass hover:border-slate-500/60'} glass-hover`}
    >
      {isRecommended && (
        <div className="absolute -top-2.5 left-4">
          <span className="bg-blue-600 text-white text-xs font-bold px-3 py-0.5 rounded-full">★ Consigliato</span>
        </div>
      )}
      ...
      <div className="text-xs text-slate-500 border-t border-slate-700 pt-3">{product.noteTecniche}</div>
  ...
  return (
    <div className="animate-fade-in space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <button onClick={() => onStart('quote')} className="glass glass-hover rounded-2xl p-6 text-left group transition-all duration-200 hover:scale-[1.02]">
          ...
          <div className="text-sm text-slate-400">Guida rapida: seleziona pavimentazione e veicolo, ottieni il modello corretto con prezzo.</div>
          <div className="mt-4 flex items-center gap-1 text-blue-400 text-sm font-medium">Inizia <ChevronRight size={16} /></div>
        </button>
        <button onClick={() => onStart('order')} className="glass glass-hover rounded-2xl p-6 text-left group transition-all duration-200 hover:scale-[1.02]">
          ...
          <div className="text-sm text-slate-400">Compila l'ordine direttamente con dati cliente, quantità e note specifiche.</div>
          <div className="mt-4 flex items-center gap-1 text-emerald-400 text-sm font-medium">Inizia <ChevronRight size={16} /></div>
        </button>
      </div>
      <div className="glass rounded-xl p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          {importStatus && <Badge text={importStatus.includes('Errore') ? importStatus : `✓ ${importStatus}`} color={importStatus.includes('Errore') ? 'amber' : 'green'} />}
        </div>
        <p className="text-xs text-slate-500 mb-3">Importa il file Excel del listino per aggiornare i prezzi netti. Le colonne attese: Riferimento, Modello, Portata, Netto Riv. (€).</p>
        <label className="block border-2 border-dashed border-slate-700 rounded-lg p-4 text-center cursor-pointer hover:border-blue-500/50 transition-colors" onDragOver={e => e.preventDefault()} onDrop={handleFileDrop}>
...
function ConfiguratorView({ mode, products, onResult, onBack }) {
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="p-2 rounded-lg glass hover:bg-slate-700 transition-colors"><ArrowLeft size={18} /></button>
        <div>
          <div className="text-xs text-slate-500 uppercase tracking-wider mb-0.5">{mode === 'order' ? 'Nuovo Ordine' : 'Nuovo Preventivo'}</div>
      ...
      {step === 0 && (
        <div className="animate-slide-up">
          ...
          <p className="text-sm text-slate-400 mb-5">La scelta del pavimento determina la famiglia di sollevatori.</p>
          <div className="space-y-3">
            {FLOOR_TYPES.map(f => (
              <button key={f.id} onClick={() => handleFloor(f.id)} className="w-full glass glass-hover rounded-xl p-5 text-left transition-all hover:scale-[1.01]">
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-lg font-bold text-white mb-1">{f.label}</div>
                    <div className="text-sm text-slate-400">{f.desc}</div>
                  </div>
                  <div className="ml-4"><Badge text={f.note} color={f.color === 'blue' ? 'blue' : 'slate'} /></div>
                </div>
              </button>
            ))}
          </div>
      ...
      {step === 1 && (
        <div className="animate-slide-up">
          <div className="flex items-center gap-2 mb-1">
            ...
            <Badge text={pavimentazione === 'industriale' ? 'Senza Pedana' : 'Con Pedana'} color={pavimentazione === 'industriale' ? 'blue' : 'slate'} />
          </div>
          <p className="text-sm text-slate-400 mb-5">Seleziona la categoria del veicolo da sollevare.</p>
          <div className="space-y-2">
            {VEHICLE_TYPES.map(v => (
              <button key={v.id} onClick={() => handleVehicle(v.id)} className="w-full glass glass-hover rounded-xl px-4 py-3.5 text-left flex items-center gap-4 transition-all hover:scale-[1.005]">
  ...
  return (
    <div className="animate-fade-in space-y-5">
      <div className="flex items-center gap-3">
        <button onClick={onBack} className="p-2 rounded-lg glass hover:bg-slate-700 transition-colors"><ArrowLeft size={18} /></button>
        <div>
          <div className="text-xs text-slate-500 uppercase tracking-wider mb-0.5">{mode === 'order' ? 'Nuovo Ordine' : 'Nuovo Preventivo'} — Risultati</div>
          <StepIndicator current={3} />
        </div>
      </div>
      <div className="glass rounded-xl p-4 flex flex-wrap gap-3">
        <div className="flex items-center gap-2 text-sm"><Building2 size={14} className="text-slate-400" /><span className="text-slate-400">Pavimento:</span><span className="text-white font-medium">{floorLabel}</span></div>
        <div className="w-px h-4 bg-slate-700" />
        <div className="flex items-center gap-2 text-sm"><Truck size={14} className="text-slate-400" /><span className="text-slate-400">Veicolo:</span><span className="text-white font-medium">{vehicleInfo?.label}</span></div>
        <div className="w-px h-4 bg-slate-700" />
        <div className="flex items-center gap-2 text-sm"><Package size={14} className="text-slate-400" /><span className="text-slate-400">Trovati:</span><span className="text-white font-medium">{results.length} modelli</span></div>
      </div>
      {results.length === 0 ? (
        <div className="glass rounded-xl p-8 text-center">
          ...
          <div className="text-sm text-slate-400 mb-4">Contatta l'ufficio tecnico.</div>
          <button onClick={onReset} className="glass glass-hover rounded-lg px-4 py-2 text-sm">Nuova ricerca</button>
        </div>
      ) : (
        <div className="space-y-4">
          {results.map((p, i) => (
            <ProductCard key={p.id} product={p} isRecommended={i === 0} onSelect={onSelectProduct} mode={mode} />
          ))}
        </div>
      )}
      <button onClick={onReset} className="w-full glass glass-hover rounded-xl py-3 flex items-center justify-center gap-2 text-sm text-slate-400 hover:text-white transition-colors">
...
function QuoteView({ mode, product, config, onBack, onReset }) {
  ...
  const waParams = { customer, product, qty, sconto, scontoEuro, prezzoTotale, prezzoFinale, note, config, mode, docType };

  const handleCopyWhatsApp = async () => {
    const text = generateWhatsAppText(waParams);
    try {
  ...
  const handleDownloadTxt = () => {
    ...
    a.href = url;
    a.download = `${docType}_${product.codice}_${customer.nome || 'cliente'}_${today().replace(/\//g, '-')}.txt`;
    a.click();
      ...
      <div className="animate-fade-in space-y-5">
        ...
        <div className="hidden print:block text-black">
          <div className="flex justify-between items-start border-b-2 border-gray-300 pb-4 mb-6">
            ...
            <div className="text-right">
              <div className="text-lg font-bold">{docType}</div>
        ...
        <div className="no-print flex items-center gap-3">
          <button onClick={() => setGenerated(false)} className="p-2 rounded-lg glass hover:bg-slate-700 transition-colors"><ArrowLeft size={18} /></button>
          <div>
        ...
        <div className="glass rounded-xl overflow-hidden print:bg-white print:text-black print:rounded-none print:border-0">
          <div className="bg-slate-800 print:bg-gray-100 p-4 border-b border-slate-700 print:border-gray-300">
            <div className="flex justify-between items-start">
              <div>
                ...
                <div className="text-white print:text-black font-bold text-lg">{customer.azienda || customer.nome || '—'}</div>
                ...
                {customer.indirizzo && <div className="text-xs text-slate-400 print:text-gray-500">{customer.indirizzo}</div>}
              </div>
              <div className="text-right">
          ...
          <div className="p-4 border-b border-slate-700 print:border-gray-300">
            <div className="flex flex-col items-end gap-1 text-sm">
              {sconto > 0 && (
                <>
                  <div className="flex gap-6 text-slate-400 print:text-gray-500"><span>Imponibile</span><span className="font-mono">{formatPrice(prezzoTotale)}</span></div>
                  <div className="flex gap-6 text-amber-400 print:text-amber-700"><span>Sconto {sconto}%</span><span className="font-mono">-{formatPrice(scontoEuro)}</span></div>
                </>
              )}
          ...
          {(note || product.noteTecniche) && (
            <div className="p-4">
              {note && <div className="mb-3"><div className="text-xs text-slate-500 print:text-gray-500 uppercase tracking-wider mb-1">Note</div><div className="text-sm text-slate-300 print:text-gray-700">{note}</div></div>}
              <div><div className="text-xs text-slate-500 print:text-gray-500 uppercase tracking-wider mb-1">Dati Tecnici</div><div className="text-xs text-slate-400 print:text-gray-600">{product.noteTecniche}</div></div>
            </div>
          )}
        </div>
        ...
        <div className="no-print grid grid-cols-2 gap-3">
          ...
          <button
            ...
            className="bg-green-600 hover:bg-green-500 text-white rounded-xl py-3 flex items-center justify-center gap-2 text-sm font-semibold transition-colors"
          >
          ...
          <button
            ...
            className={`${copied ? 'bg-emerald-700' : 'glass glass-hover'} text-white rounded-xl py-3 flex items-center justify-center gap-2 text-sm font-medium transition-colors`}
          >
          ...
          <button
            ...
            className="glass glass-hover text-slate-300 hover:text-white rounded-xl py-3 flex items-center justify-center gap-2 text-sm transition-colors"
          >
  ...
  return (
    <div className="animate-fade-in space-y-5">
      ...
      <div className="glass rounded-xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <input className={inputCls} type="number" min="1" max="99" value={qty} onChange={e => setQty(Math.max(1, parseInt(e.target.value) || 1))} />
          </div>
          <div>
            <label className="text-xs text-slate-400 mb-1 block">Sconto % (opzionale)</label>
            <input className={inputCls} type="number" min="0" max="50" placeholder="0" value={sconto || ''} onChange={e => setSconto(Math.min(50, Math.max(0, parseFloat(e.target.value) || 0)))} />
          </div>
        </div>
        <textarea className={`${inputCls} resize-none h-20`} placeholder="Note aggiuntive, condizioni speciali..." value={note} onChange={e => setNote(e.target.value)} />
      </div>
      ...
      <div className="glass rounded-xl p-4">
        ...
        {sconto > 0 && (
          <div className="flex justify-between items-center text-sm text-amber-400 mb-1">
            <span>Sconto {sconto}%</span><span>-{formatPrice(scontoEuro)}</span>
          </div>
        )}
      ...
      {!customer.nome && <p className="text-xs text-slate-500 text-center">* Inserisci almeno il nome cliente per procedere</p>}
    </div>
  );
}
    ...
    <div className="min-h-screen bg-navy-900">
      <header className="sticky top-0 z-40 glass border-b border-slate-800/60 no-print">
        <div className="max-w-lg mx-auto px-4 h-14 flex items-center justify-between">
          <Logo />
          <button onClick={handleReset} className="text-xs text-slate-400 hover:text-white flex items-center gap-1 transition-colors">
            <RotateCcw size={13} /> Reset
          </button>
        </div>
      </header>
      <main className="max-w-lg mx-auto px-4 py-6 pb-20">
        {view === 'dashboard' && <DashboardView onStart={handleStart} onImport={handleImport} importStatus={importStatus} />}
        {view === 'configurator' && <ConfiguratorView mode={mode} products={products} onResult={handleConfigResult} onBack={() => setView('dashboard')} />}
        {view === 'results' && config && <ResultsView mode={mode} config={config} onSelectProduct={handleSelectProduct} onBack={() => setView('configurator')} onReset={handleReset} />}
        {view === 'quote' && selectedProduct && config && <QuoteView mode={mode} product={selectedProduct} config={config} onBack={() => setView('results')} onReset={handleReset} />}
      </main>
    </div>
  );
}
